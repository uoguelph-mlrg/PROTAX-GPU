...
int compute_cnode_probs_best2(TaxonomyNode *node, int nid, double prevprob, SequenceSet *rseq, Model *m, double **scs, char *seq, double pth) {
  ...
  // min 2 distances, pad extra space with 1.0
  for (i=0; i<node[nid].num_cnodes; i++) {
    ...
    // known, with reference sequences
    ...
    // known, without reference sequences
    else {
  ...
  // softmax applied to get branch probs
  ezsum = 1e-100;
  for (i=0; i<node[nid].num_cnodes; i++) {
    cid = node[nid].cnode_index[i];
...
// getting cumulative prob for each child
  node[nid].sumcprob_no_rseqs = 0.0;
  for (i=0; i<node[nid].num_cnodes; i++) {
    cid = node[nid].cnode_index[i];
    node[cid].prob *= prevprob;             // chaining branch probs (product)
